...
export const AutoCompleteChips = ({ Data, listOnClick, chipDeleteOnClick, onChange, chipsData = [], width = 20, height = 2, borderRadius = 5, borderWidth = 1, borderColor = 'black', chipBgColor = 'gray', chipTextColor = 'black', chipfontSize = 18, chipMargin = 1, crossFillColor = 'black', placeholder = 'Type and press Enter', }) => {
    ...
    return (React.createElement("div", { style: container, className: 'container' },
        ...
        React.createElement("input", { style: textinput, className: 'textinput', placeholder: placeholder, type: 'text', value: inputValue, onChange: (e) => {
                onChange(e.target.value);
                setInputValue(e.target.value);
            } }),
        inputValue.trim() !== '' ? (filteredData.length > 0 ? (filteredData.map((e) => (React.createElement("p", { className: 'dropdown', key: e.id, onClick: () => {
                if (!chips.some((v) => v.id === e.id)) {
                    setChips((prevChips) => [...prevChips, { label: e.label, id: e.id }]);
                }
                setInputValue('');
                listOnClick(e);
            } }, e.label)))) : (React.createElement("p", { className: 'dropdown' }, Data && Data.length > 0 ? 'No options' : 'Add data'))) : null));
};
